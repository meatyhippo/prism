 ...
 * can read and write chores, tasks, events, shopping lists, messages,
 * meals, goals, and more.
 *
...
if (!BASE_URL || !TOKEN) {
    process.stderr.write('Error: PRISM_BASE_URL and PRISM_API_TOKEN environment variables are required.\n');
    process.exit(1);
}
// ---------------------------------------------------------------------------
// HTTP helper
...
// CHORES
// ===========================================================================
server.tool('list_chores', 'List all chores. Optionally filter by assignedTo (user UUID) or enabled status.', {
...
server.tool('create_chore', 'Create a new chore.', {
...
server.tool('update_chore', 'Update an existing chore by ID.', {
...
server.tool('delete_chore', 'Delete a chore by ID.', { id: z.string().uuid().describe('Chore UUID') }, async ({ id }) => ok(await api('DELETE', `/api/chores/${id}`)));
server.tool('complete_chore', 'Mark a chore as completed by a specific user.', {
    id: z.string().uuid().describe('Chore UUID'),
...
server.tool('update_task', 'Update an existing task by ID.', {
    id: z.string().uuid(),
    title: z.string().min(1).max(255).optional(),
    description: z.string().optional(),
    assignedTo: z.string().uuid().nullable().optional(),
...
server.tool('delete_task', 'Delete a task by ID.', { id: z.string().uuid() }, async ({ id }) => ok(await api('DELETE', `/api/tasks/${id}`)));
// ===========================================================================
// CALENDAR EVENTS
// ===========================================================================
server.tool('list_events', 'List calendar events within a date range.', {
...
server.tool('create_event', 'Create a new calendar event.', {
    title: z.string().min(1).max(255),
...
server.tool('update_event', 'Update an existing calendar event.', {
    id: z.string().uuid(),
    title: z.string().min(1).max(255).optional(),
...
server.tool('delete_event', 'Delete a calendar event by ID.', { id: z.string().uuid() }, async ({ id }) => ok(await api('DELETE', `/api/events/${id}`)));
// ===========================================================================
// SHOPPING
// ===========================================================================
server.tool('list_shopping_lists', 'List all shopping lists.', {}, async () => ok(await api('GET', '/api/shopping-lists')));
server.tool('list_shopping_items', 'List shopping items, optionally filtered by list.', {
...
server.tool('add_shopping_item', 'Add an item to a shopping list.', {
...
server.tool('update_shopping_item', 'Update a shopping item (e.g. check it off).', {
    id: z.string().uuid(),
...
server.tool('delete_shopping_item', 'Remove an item from a shopping list.', { id: z.string().uuid() }, async ({ id }) => ok(await api('DELETE', `/api/shopping-items/${id}`)));
// ===========================================================================
// MESSAGES (family message board)
// ===========================================================================
server.tool('list_messages', 'List family messages from the message board.', {
...
server.tool('post_message', 'Post a new message to the family message board.', {
...
server.tool('delete_message', 'Delete a message by ID.', { id: z.string().uuid() }, async ({ id }) => ok(await api('DELETE', `/api/messages/${id}`)));
// ===========================================================================
// FAMILY
// ===========================================================================
server.tool('list_family', 'List all family members with their UUIDs, roles, and colors.', {}, async () => ok(await api('GET', '/api/family')));
// ===========================================================================
// MEALS
// ===========================================================================
server.tool('list_meals', 'List meal plan entries.', {
...
server.tool('create_meal', 'Add a meal to the meal plan.', {
    name: z.string().min(1).max(255),
...
server.tool('delete_meal', 'Remove a meal from the plan.', { id: z.string().uuid() }, async ({ id }) => ok(await api('DELETE', `/api/meals/${id}`)));
// ===========================================================================
// GOALS
// ===========================================================================
server.tool('list_goals', 'List family goals (chore-point reward targets).', {}, async () => ok(await api('GET', '/api/goals')));
server.tool('create_goal', 'Create a new goal.', {
    title: z.string().min(1).max(255),
    description: z.string().optional(),
...
// WEATHER
// ===========================================================================
server.tool('get_weather', 'Get current weather conditions and today\'s forecast.', {}, async () => ok(await api('GET', '/api/weather')));
// ===========================================================================
// RECIPES
// ===========================================================================
server.tool('list_recipes', 'List saved recipes.', {}, async () => ok(await api('GET', '/api/recipes')));
server.tool('import_recipe_url', 'Import a recipe from a URL.', {
    url: z.string().url().describe('URL of the recipe page to import'),
}, async ({ url }) => ok(await api('POST', '/api/recipes/import-url', { url })));
// ===========================================================================
// MAINTENANCE
// ===========================================================================
server.tool('list_maintenance', 'List home maintenance items.', {}, async () => ok(await api('GET', '/api/maintenance')));
server.tool('create_maintenance_item', 'Create a home maintenance item.', {
    title: z.string().min(1).max(255),
    description: z.string().optional(),
...
// POINTS
// ===========================================================================
server.tool('get_points', 'Get current chore point totals for all family members.', {}, async () => ok(await api('GET', '/api/points')));
// ---------------------------------------------------------------------------
// Start
// ---------------------------------------------------------------------------
